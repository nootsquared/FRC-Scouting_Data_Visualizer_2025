...
export function FrostedBarCursor({
  ...
  const bottomY = paddedY + paddedHeight;
  const leftX = paddedX;
  const rightX = paddedX + paddedWidth;
  const controlRadius = cornerRadius;

  const outlinePath = [
    `M ${leftX} ${bottomY}`,
    `L ${leftX} ${paddedY + controlRadius}`,
    `Q ${leftX} ${paddedY} ${leftX + controlRadius} ${paddedY}`,
    `L ${rightX - controlRadius} ${paddedY}`,
    `Q ${rightX} ${paddedY} ${rightX} ${paddedY + controlRadius}`,
    `L ${rightX} ${bottomY}`,
    'Z',
  ].join(' ');
  ...
  return (
    <Layer>
      ...
      <g clipPath={`url(#${clipId})`}>
        ...
        <rect
          x={paddedX + 10}
          y={paddedY + paddedHeight - bottomAccentYOffset - 2}
